...
export interface GenerateTradeOptions extends CreateTradeOptions {
  addressId: string;
  fromAmount: number;
  toAmount: number;
}

export const generateTrade = (wallet: Wallet, options: GenerateTradeOptions): Trade => {
  return new Trade(generateTradeData(wallet, options));
};

export const generateTradeData = (wallet: Wallet, options: GenerateTradeOptions) => {
  return {
